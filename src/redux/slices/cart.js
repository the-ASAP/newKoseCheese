...
const { addProduct, removeProduct, incProductCount, decProductCount, purchaseOrder, putProducts } =
  cartSlice.actions;
    ...
    try {
      await APIBitrix.post('basket/remove/', {
        ...
        if (res.code === 200) {
          ...
          throw new Error(
            'Произошла ошибка при удалении товара. Попробуйте обновить страницу и удалить товар еще раз'
          );
        }
      });
    } catch ({ message }) {
  ...
  async (_, { dispatch, getState }) => {
    ...
    try {
      await APIBitrix.post('basket/items/', {
        fuser_id: user.user_id || localStorage.getItem('fuser_id')
      }).then((res) => {
        if (res.code === 200) {
          dispatch(putProducts(res.data || []));
        } else {
