      ...
      <div className={s.container}>
        <div className={s.block}>
          {isClientSide && windowSize >= 768 ? (
            ...
            <div className={s.slider}>
              <Swiper className="swiper-product" spaceBetween={15} pagination>
                {gallery.map((el) => (
                  <SwiperSlide key={el}>
        ...
        <div className={s.info}>
          ...
          <div className={s.composition}>
            ...
            {detailText && <Accordion title={'Описание'}>{parse(detailText)}</Accordion>}
            {storage && <Accordion title={'Особенности хранения'}>{parse(storage)}</Accordion>}
          </div>
        </div>
      </div>
    </>
