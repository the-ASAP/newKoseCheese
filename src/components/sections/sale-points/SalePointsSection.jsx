...
export const SalePointsSection = () => {
  ...
  const [activeCity, setActiveCity] = React.useState(cities.find((city) => city.id === activeId));
      ...
      <Wrapper>
        ...
        {isClientSide && windowSize >= 768 ? (
          <div className={s.points}>
            {activeCity.points.length > 0 &&
              activeCity.points.map((point) => (
                <MarketCard key={point.id} city={activeCity.title} {...point} />
              ))}
          </div>
        ) : (
          <Swiper slidesPerView={1.35} spaceBetween={20}>
            {activeCity.points.length > 0 &&
              activeCity.points.map((point) => (
                <SwiperSlide key={point.phone}>
                  <MarketCard city={activeCity.title} {...point} />
                </SwiperSlide>
              ))}
          </Swiper>
        )}
